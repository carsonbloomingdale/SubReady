...
PARSER_TYPE_TO_SLOT: dict[str, str] = {
...
def parsed_to_api_dict(parsed: ParsedDocument) -> dict:
    """Compact parse payload for API responses."""
    return {
        "detectedTypes": parsed.detected_types,
        "dates": parsed.dates,
        "emails": parsed.emails,
        "amounts": parsed.amounts,
        "policyOrLicenseIds": parsed.policy_or_license_ids,
        "parseWarnings": parsed.parse_warnings,
        "lineCount": parsed.line_count,
        "sections": [
            {"title": s.title, "lineCount": len(s.lines), "preview": s.lines[:6]}
            for s in parsed.sections
        ],
    }


def infer_slot_from_parsed(parsed: ParsedDocument) -> str | None:
    for doc_type in parsed.detected_types:
        slot = PARSER_TYPE_TO_SLOT.get(doc_type)
        if slot:
            return slot
    return None
